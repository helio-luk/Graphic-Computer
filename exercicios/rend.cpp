...
void drawSurface_flat(){
	...
	for(int i = 1; i <= SIZE_GAUSSIAN; i++){
		for(int j = 1; j <= SIZE_GAUSSIAN; j++){
			glNormal3f(gaussian[i+1][j].ponto.x, gaussian[i+1][j].ponto.y,gaussian[i+1][j].ponto.z);
...
void desenhaCena() {
    ...
    /* Desenha os objetos na cena utilizando o modelo de gourad*/
    drawSurface_Gourard();
